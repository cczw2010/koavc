...
export default async function(config){
  ...
  // ====初始化静态服务，
  const staticPaths = []
  // (vue & 自定义)放上面,下面的中间件就不会响应静态服务的内容了
  ...
  // 另外需要忽略静态目录路径
  app.use(routeGuard(config.app,staticPaths,logger))
  // 加载全局中间件,失败将退出应用
